...
public class PhotoManager {
	...
	public int findPhoto(String photoSource) {
		...
		for (int i = 0; i < allPhotos.size(); i++) {
			if (allPhotos.get(i).getPhotoSource() == photoSource) {
				return i;
			}
		}
		return -1;
	...
	public boolean addComment(String photoSource, String newComment) {
		if (photoSource == null || newComment.isBlank()) {
			return false;
		} else if (findPhoto(photoSource) >= 0) {
	...
	public String getComments(String photoSource) {
		if (photoSource == null || findPhoto(photoSource) < 0) {
			return null;
		}
	...
	public boolean loadPhotos(String filename) {
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				String address = text.substring(0, spaces[0]), width = text.substring(spaces[0] + 1, spaces[1]),
						height = text.substring(spaces[1] + 1, spaces[2]),
						date = text.substring(spaces[2] + 1, text.length());
				int widthInt = Integer.parseInt(width), heightInt = Integer.parseInt(height);
				Photo newPhoto = new Photo(address, widthInt, heightInt, date);
				allPhotos.add(newPhoto);
			}
